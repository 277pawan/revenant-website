...
function ProviderIcon({ id }: { id: AuthProviderInfo["id"] }) {
  ...
  return (
    <svg className="h-4 w-4 text-foreground-muted" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
...
export function OAuthButtons({ providers, onUnavailable, onSuccess, onError }: Props) {
  ...
  async function handleClick(provider: AuthProviderInfo) {
    ...
    try {
      const popup = openOAuthPopup(provider.authorizePath);
      if (!popup) {
